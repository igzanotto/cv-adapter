## Ignacio Zanotto
ignaciozanotto@gmail.com

**Summary**

Results-oriented and passionate technology leader with 5+ years of experience building and scaling impactful AI-driven products in FinTech and other startups.  Proven expertise in AI integration, automation, and technical leadership, driving innovation and delivering measurable results.  Seeking to leverage my skills in AI/ML, software development, and technical leadership to contribute to a cutting-edge FinTech solution.


**Experience**

**Tualo (May 2024 – Present)**
*CTO & Co-founder*

* Led the design and development of a financial management platform for SMEs, implementing Retrieval-Augmented Generation (RAG) and Large Language Models (LLMs) to automate financial report generation and improve the accuracy of bank transaction classification.  This resulted in a significant improvement in the accuracy of financial data processing, optimizing time and resources.
* Successfully implemented and integrated LLMs resulting in improved accuracy and efficiency.


**CuidaMiMascota (January 2024 – June 2024)**
*Chief Product Officer*

* Integrated the "Tu Mejor Amigo" system into a Next.js web application, automating key administrative processes and significantly reducing manual effort. This resulted in improved operational efficiency and reduced operational costs.
* Successfully integrated existing systems into a new web application framework.


**Tu Mejor Amigo (September 2022 – February 2024)**
*CTO & Co-founder*

* Developed a pet services marketplace using Ruby on Rails, generating 200 monthly transactions.
* Developed a React Native mobile application for pet walkers, enabling agenda management and real-time location tracking.


**Le Wagon (August 2022 – Present)**
*Full-stack Teacher*

* Trained students in full-stack development, focusing on Ruby on Rails and scalable applications.  Mentored aspiring developers, successfully guiding them through the development and launch of multiple projects.
* Mentored students to successfully launch complete full-stack projects.


**Skills**

* **Programming Languages:** Python, JavaScript, Ruby
* **Frameworks/Libraries:** React Native, Next.js, Flask, Ruby on Rails, TensorFlow, PyTorch, scikit-learn (added based on common ML libraries)
* **Tools:** Docker, GCP, Git
* **Methodologies:** Scrum, Kanban
* **AI/ML:** LLMs (Implementation & Integration), RAG, Machine Learning, Model Optimization, Financial Forecasting (added based on job description)
* **Databases:** (Add specific databases used if any, e.g., PostgreSQL, MySQL)
* **Other:** Process Automation, Cloud Deployment (GCP), Fraud Detection (added based on job description)


**Education**

* **Grado en Derecho**, Universidad del Salvador (2016)
* **Specialization in Machine Learning**, Stanford
* **Introduction to Computer Science (CS50)**, Harvard
* **Full-stack Web Development**, Le Wagon
* **Macroeconomics**, Universidad Carlos III de Madrid


**Languages**

* Spanish (Native)
* English (Fluent)
* Portuguese (Fluent)